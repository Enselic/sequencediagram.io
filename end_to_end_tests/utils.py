...
def driver(request):
    ...
    write_code_coverage_data_if_present(driver)
    driver.quit()
...
def create_chrome_driver():
    ...
    options.add_argument("window-size={},{}".format(WINDOW_SIZE_WIDTH,
                                                    WINDOW_SIZE_HEIGHT))
    setup_common_options(options)
...
def setup_common_options(options):
    ...
    # environments
    if is_truthy(os.environ.get('HEADLESS', os.environ.get('CI'))):
...
def write_code_coverage_data_if_present(driver):
    # Get coverage data from browser
    __coverage__ = driver.execute_script(
